...
async function run() {
  ...
  try {
    ...
    const willExpire = await cmd(
      `openssl x509 -checkend 86400 -in ${dir}/ca.crt`
    );
  ...
  if (genCerts) {
    ...
    await cmd(
      `openssl req -batch -new -key ${dir}/ca.key -out ${dir}/ca.csr -config ${dir}/config.txt`
    ); // gen CSR
    await cmd(
      `openssl x509 -req -days 2 -in ${dir}/ca.csr -out ${dir}/ca.crt -signkey ${dir}/ca.key`
    ); // sign

    await cmd(`openssl genrsa -out ${dir}/server.key 1024`); // gen CA key
    await cmd(
      `openssl req -batch -new -key ${dir}/server.key -out ${dir}/server.csr -config ${dir}/config.txt`
    ); // gen CSR
    await cmd(
      `openssl x509 -req -days 2 -in ${dir}/server.csr -signkey ${dir}/server.key -out ${dir}/server.crt`
    ); // sign
  } else {
  ...
  console.log(`Server listening on port ${port} with root ${root}`);
}
